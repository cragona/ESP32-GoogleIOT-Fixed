...
void GIOT_setup() {
  ...
  Serial.println("Setup GIOT.....May Crash =]");
  //dht.begin();
...
unsigned long previousRead = 0, currentRead = 0, timerToPublish = 0;
...
{ 
    if (gps.location.isValid()) 
    {
        ...
        Serial.print("Lat degree: "); Serial.print(latitude); // Latitude in degrees (double)
        Serial.print(" Long degree: "); Serial.println(longitude); // Longitude in degrees (double)

        Serial.print("Altitude meters: "); Serial.println(gps.altitude.meters()); // Altitude in meters (double)
        // delay
        float massPartical = read_all();

        //print timer
        previousRead = currentRead;
        currentRead = millis();
        timerToPublish += currentRead - previousRead;
        ...
        // format below
        // deviceId: "nomad-9999-00000001",
        // humidity: 94,
        // latitude: 45.004,
        // longitude: -92.9994,
        // pm2x5Mass: 300,
        // temperature: 5,
        // timestamp: 2021-11-04T22:00:00Z

        if (timerToPublish > 5000) 
        {
          timerToPublish = 0; //reset, publish every 5 seconds or so
          ...
          doc["pm2x5Mass"] = massPartical;
          doc["temperature"] = temp;
          doc["timestamp"] = gps.time.value();
